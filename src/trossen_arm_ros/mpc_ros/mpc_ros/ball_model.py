...
def _predict_numba(
    ...
    while (pos[0] > stop_x) and (y_min <= pos[1] <= y_max):
        ...
        if (pos[2] - ball_radius) <= table_surface_z and pos[0] >= table_edge and n - bounce_n > bounce_cooldown:
            bounce_n = n
...
class BallModel:
    def __init__(self, friction_coeff=0.25, restitution_coeff=0.93, ball_radius=0.02, k_M=0.0036, k_D=0.12, g=9.81):
    ...
    def predict(self, pos_init, vel_init, ang_vel_init, dt=0.001, stop_x=hit_plane, use_numba=True):
        # if vel_init[0] <= 0:
        #     return None, None
        if use_numba:
            return _predict_numba(
                pos_init, vel_init, ang_vel_init,
                dt, stop_x, table_x - table_length / 2,
    ...
    def solve_landing(self,
        ...
        # ocp_solver.add_eq_constraint(pos_hit[0], float(p_land[0]))
        # ocp_solver.add_eq_constraint(pos_hit[1], float(p_land[1]))
        
        ocp_solver.add_leq_constraint(cs.sumsqr(pos_hit[0:2] - cs.MX(p_land[0:2])), 1e-4)  # Allow some small tolerance

        # (Optional) if you want p_land[2] to be respected too, you can enforce it:
        # ocp_solver.add_eq_constraint(pos_hit, cs.MX(p_land))
        ...
        # Optional: encourage earlier landing (helps avoid very long t with tiny velocities)
        # ocp_solver.add_cost(1e-3 * t_hit**2)
        ...
        # print('Pamaters:')
        # print('p_des:', p_des, 'p_land:', p_land)
        # print('Pre-impact vel:', vel_pre_impact, 'ang_vel:', ang_vel_pre_impact)
        # print('Optimal v_des:', v_des_opt)
        # print('Optimal n_des:', n_des_opt)
